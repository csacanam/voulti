...
export const CountdownTimer: React.FC<CountdownTimerProps> = ({
  ...
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      ...
      <span className="text-gray-500 text-sm">{t.countdown.timeRemaining}</span>
      <div className={`font-mono font-medium ${getTimeColor()}`}>
        {timeLeft.hours > 0 && (
          <>
            <span>{formatTime(timeLeft.hours)}</span>
            <span className="text-gray-400">{t.countdown.hours}</span>
            <span className="mx-1">:</span>
          </>
        )}
        <span>{formatTime(timeLeft.minutes)}</span>
        <span className="text-gray-400">{t.countdown.minutes}</span>
        <span className="mx-1">:</span>
        <span>{formatTime(timeLeft.seconds)}</span>
        <span className="text-gray-400">{t.countdown.seconds}</span>
      </div>
    </div>
  );
};
