 ...
 * `currentColor` — the same shape works green on the white header and white on
...
interface VoultiMarkProps {
  className?: string;
  /** Give the mark a name when it stands alone. Leave it out beside a wordmark. */
  label?: string;
}

export const VoultiMark: React.FC<VoultiMarkProps> = ({ className = 'w-7 h-7', label }) => (
  <svg
    viewBox="0 0 512 512"
    fill="none"
    className={className}
...
/** Mark plus wordmark, the way it appears in every header. */
export const VoultiLogo: React.FC<{ className?: string }> = ({ className = '' }) => (
