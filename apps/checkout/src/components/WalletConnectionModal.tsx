...
export const WalletConnectionModal: React.FC<WalletConnectionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-50 rounded-xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden max-w-md w-full max-h-[90vh]">
        {/* Header */}
        ...
        {(() => {
          return isConnected && address ? (
            <div className="p-6 border-b border-gray-200 bg-white/50 flex-shrink-0">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-red-600 hover:bg-red-700 text-gray-900 text-sm rounded-md transition-colors duration-200"
                >
                  Desconectar
                </button>
              </div>
          ...
          {isConnected ? (
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-red-600 hover:bg-red-700 text-gray-900 font-medium py-3 px-4 rounded-lg transition-colors duration-200"
              >
                Desconectar Wallet
              </button>
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {availableWallets
                      ...
                      .map((wallet) => (
                           ...
                           className="flex flex-col items-center space-y-2 p-4 bg-white hover:bg-gray-100 rounded-lg transition-colors duration-200 border border-gray-200 hover:border-gray-300"
                         >
                           <div className="w-12 h-12 flex items-center justify-center">
                             {getWalletLogo(wallet.id, 32)}
                           </div>
                           <span className="text-sm font-medium text-gray-900">{wallet.shortName}</span>
                         </button>
                      ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {connectors.map((connector) => (
                         ...
                         className="w-full flex items-center justify-between p-3 bg-white hover:bg-gray-100 rounded-lg transition-colors duration-200 border border-gray-200 hover:border-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                       >
                         <div className="flex items-center space-x-3">
                  ...
                  {/* Install Options */}
                  <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wide mt-4">
                    Instalar Wallet
                  </h3>
                  <div className="space-y-2">
                    {availableWallets
                      ...
                      .map((wallet) => (
                           ...
                           className="w-full flex items-center justify-between p-3 bg-white hover:bg-gray-100 rounded-lg transition-colors duration-200 border border-gray-200 hover:border-gray-300"
                         >
                           <div className="flex items-center space-x-3">
                             <div className="w-6 h-6 flex items-center justify-center">
                               {getWalletLogo(wallet.id, 20)}
                             </div>
                             <span className="text-gray-900 font-medium">Instalar {wallet.shortName}</span>
                           </div>
